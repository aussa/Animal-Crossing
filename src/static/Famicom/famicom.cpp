...
static void SetupInternalCommentImage(u8* data) {
    ...
    famicomCommon.memcard_game_header.comment_img_size = (u32)((uintptr_t)data - (uintptr_t)data_p);
}
...
static s32 memcard_data_load(
    ...
    for (i = 0; i < 2; i++) {
        ...
        // Reading successful!
        OSReport("読み込み成功！！\n");
        FamicomSaveDataHeader* read_save_header = (FamicomSaveDataHeader*)((uintptr_t)buf + status.offsetData);
...
static int SetupResBanner(const ResTIMG* img, u8* dst, size_t max_size, size_t* size, u8* type) {
    ...
    if (size != nullptr) {
        *size = (u32)((uintptr_t)data_p - (uintptr_t)dst);
    }
...
static int SetupResIcon(const ResTIMG* img, u8* dst, size_t max_size, size_t* size_p, u16* icon_fmt_p, u16* icon_flags_p) {
    ...
    if (size_p != nullptr) {
        *size_p = (u32)((uintptr_t)data_p - (uintptr_t)dst);
    }
